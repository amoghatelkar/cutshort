...
export const Third: FC<IThird> = ({ setStep }) => {
    ...
    const card = (text: string, description: string) =>
        <React.Fragment>
            <CardContent>
                {text === 'Myself' ? <MyselfIcon color="primary" sx={{ fontSize: 40 }} /> : <TeamIcon color="primary" sx={{ fontSize: 40 }} />}
    ...
    return (<>
        <Box sx={{ width: '100%', paddingTop: '40px', textAlign: 'center' }}>
            <h2 style={{ fontFamily: 'Inter', fontWeight: 'bolder', margin:'5px 0'}}> {"How are you planning to use Eden?"}</h2>
            <p style={{ color: 'grey', fontFamily: 'Inter', fontWeight: 'bolder', margin: '10px 0 20px', fontSize:'smaller' }}>{"We'll streamline your setup experience accourdingly."}</p>
        </Box>
